...
def store_news_articles(articles):
    ...
    insert_article_query = "insert into articles(id, victim_name, title, publication_date, source, body) values (?, ?, ?, ?, ?, ?);"
    with get_conn(acs) as conn:
        cursor = conn.cursor()
        for article in net_new_articles:
            # create a new entry in the DB for a new article
            cursor.execute(insert_article_query, article["id"], article["name"], article["title"], article["publication_date"], article["source"], article["body"])
        conn.commit()
